...
class _ProdusenAduanState extends State<ProdusenAduan> {
  ...
  @override
  void initState() {
    _streamUsersCollection = _usersCollection.doc(userProdusenID).snapshots();
    ...
    return StreamBuilder(
      ...
      builder:(context, AsyncSnapshot snapshot) {
        if(snapshot.connectionState == ConnectionState.active){
          ...
          premiumProdusen = usersSnapshot.get('premium');
          if(premiumProdusen == 'y')
          {return Scaffold(
                                ...
                                builder: (context, snapshot) {
                                  ...
                                  if (snapshot.hasData){
                                    ...
                                    return 
                                      Container(
                                        child: Column(
                                          crossAxisAlignment: _divider_CrossAlignment(chat["id_produsen"], chat["id_peternak"], userProdusenID),
                                          children: [
                                            Container(
                                              margin: EdgeInsets.fromLTRB(10, 10, 10, 0),
                                              padding: EdgeInsets.fromLTRB(10, 0, 10, 0),
                                              decoration: BoxDecoration(
                                                color: _divider_color(chat["id_produsen"], chat["id_peternak"], userProdusenID),
...
_divider_CrossAlignment(id_produsen, id_peternak, current_id_produsen){
...
_divider_color(id_produsen, id_peternak, current_id_produsen){
  if (id_produsen == current_id_produsen){
    return Color.fromARGB(255, 196, 251, 198);
  }

  else if (id_peternak != "") {
    return Color.fromARGB(255, 175, 243, 255);
  }
  else {
